...
var BaseResolver, EventEmitter, Module, ResolverSet, ResolverShortcuts, XMLHttpRequest, abs, damerauLevenshtein, extend, idCounter, isArray, min, pow, rankSearchResults, spaceNormalizeRe, tokenNormalizeRe, tokenize, uniqueId, urlencode, xhrGET,
  __slice = [].slice,
...
rankSearchResults = function(results, query) {
  ...
  for (_i = 0, _len = results.length; _i < _len; _i++) {
    result = results[_i];
    rank1 = distance(tokenize(("" + result.artist + " " + result.track).toLowerCase()), qTokens);
    rank2 = distance(tokenize(("" + result.track + " " + result.artist).toLowerCase()), qTokens);
