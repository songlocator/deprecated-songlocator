...
var BaseResolver, EventEmitter, Module, ResolverSet, ResolverShortcuts, XMLHttpRequest, abs, bagOfWords, computeTensor, extend, idCounter, isArray, ngrams, norm, normBagOfWords, pow, rankSearchResults, spaceNormalizeRe, sumArray, tokenNormalizeRe, tokenize, uniqueId, urlencode, xhrGET,
  __slice = [].slice,
...
rankSearchResults = function(results, query, ngramRank) {
  var queryT, result, _i, _len, _results;
  queryT = computeTensor(query, ngramRank);
  _results = [];
  for (_i = 0, _len = results.length; _i < _len; _i++) {
    result = results[_i];
    _results.push(result.rank = norm(computeTensor(шеуь, ngramRank), queryT));
  }
  return _results;
};
